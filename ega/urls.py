...
urlpatterns = [
    ...
    re_path(
        r'^l/(?P<lang>en|es)/$',
        ega.views.switch_language,
        name='switch-language',
    ),
    path('logout/', ega.views.logout, name='logout'),
    path('profile/', ega.views.profile, name='profile'),
    path(
        'profile/verify/<str:email>/',
        ega.views.verify_email,
        name='verify-email',
    ),
    path('invite/', ega.views.invite_friends, name='ega-invite'),
    path(
        '<slug:slug>/invite/<slug:league_slug>/',
        ega.views.invite_friends,
        name='ega-invite-league',
    ),
    path(
        'join/<slug:slug>/<str:key>/', ega.views.friend_join, name='ega-join'
    ),
    path(
        'join/<slug:slug>/<str:key>/<slug:league_slug>/',
        ega.views.friend_join,
        name='ega-join',
    ),
    path('<slug:slug>/', ega.views.home, name='ega-home'),
    path(
        '<slug:slug>/update-champion',
    ...
    path(
        '<slug:slug>/league/<slug:league_slug>/',
        ega.views.league_home,
        name='ega-league-home',
    ),
    path(
        '<slug:slug>/matches/', ega.views.next_matches, name='ega-next-matches'
    ),
    path(
        '<slug:slug>/matches/<int:match_id>/',
        ega.views.match_details,
        name='ega-match-details',
    ),
    path('<slug:slug>/ranking/', ega.views.ranking, name='ega-ranking'),
    path(
        '<slug:slug>/ranking/f/<str:round>/',
        ega.views.ranking,
        name='ega-ranking',
    ),
    path(
        '<slug:slug>/ranking/<slug:league_slug>/',
        ega.views.ranking,
        name='ega-league-ranking',
    ),
    path(
        '<slug:slug>/ranking/<slug:league_slug>/f/<str:round>/',
        ega.views.ranking,
        name='ega-league-ranking',
    ),
